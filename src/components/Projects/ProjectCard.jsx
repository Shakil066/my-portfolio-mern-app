...
const ProjectCard = ({ card }) => {
    // console.log(card, 'card inside projectcard');
    const { id, project_id, title, photo, github_url } = card;
            ...
            {/* <div className="hover:cursor-pointer" data-aos="zoom-in"> */}
                ...
                <div className="w-full min-h-[100px] bg-white rounded-b-lg p-4 drop-shadow-md">
                    ...
                    <div className="flex items-center justify-center py-3">
                        <Link to={`/details/${id}`} className="mt-6 flex items-center justify-center w-full bg-[#db332a] py-3 px-6 text-center  text-white shadow-md shadow-pink-500/20 transition-all hover:shadow-lg hover:shadow-pink-500/40 focus:opacity-[0.85] focus:shadow-none active:opacity-[0.85] active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none">
                            <FaEye className="text-xl"></FaEye>
                            <span>View Detatails</span>
                        </Link>
                    </div>
                </div>
